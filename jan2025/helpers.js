...
const migrateFlowData = (flowData) => {
  ...
  Object.entries(flowData).forEach(([nodeId, nodeData]) => {
    ...
    if (nodeData?.["data"]?.["type"] === 200) {
      ...
      if (nodeData?.["data"]?.["val"] && Object.keys(flagChanges).includes(nodeData["data"]["val"])) {
        const current = nodeData["data"]["val"];
        newFlowData[nodeId]["data"]["val"] = flagChanges[current];

        // TODO some additionally need "text" updated
      }
    }

    // Filter flagset category change
    if (nodeData?.["data"]?.["type"] === 500 && nodeData?.["data"]?.["category"] === "Listed building consent") {
      newFlowData[nodeId]["data"]["category"] = "Works to listed buildings";
    }

    // Article 4s (options and selectable planning constraints)
    if (nodeData?.["data"]?.["type"] === 200 && nodeData?.["data"]?.["val"]?.startsWith("article4")) {
      newFlowData[nodeId]["data"]["val"] = nodeData["data"]["val"].replace("article4", "articleFour");
    }

    if (
      nodeData?.["data"]?.["type"] === 11 &&
